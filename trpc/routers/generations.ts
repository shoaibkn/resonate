...
import { TEXT_MAX_LENGTH } from "@/components/features/text-to-speech/data/constants";
  ...
  create: orgProcedure
    .input(
      z.object({
        ...
        repetitionPenalty: z.number().min(1).max(2).default(1.3),
      }),
    )
    .mutation(async ({ ctx, input }) => {
      const voice = await prisma.voice.findUnique({
        where: {
          id: input.voiceId,
          OR: [{ variant: "CUSTOM", orgId: ctx.orgId }, { variant: "SYSTEM" }],
        },
        select: {
          id: true,
          name: true,
